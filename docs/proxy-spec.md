...
GET /health                 -> liveness самого proxy
GET /ready                  -> readiness proxy + dependencies
...
Публичный contract proxy ограничен поверхностью:
...
`/agent` не публикуется наружу и не должен попадать в SPA fallback.
...
Если proxy DB или backend `/health` недоступны, readiness должен возвращать `503 Service Unavailable`.

---

## 6.3. `POST /auth/welcome/verify`
...
SOUZ_BACKEND_PROXY_TOKEN=replace-with-random-backend-proxy-token-32-plus-chars

SESSION_HASH_SECRET=replace-with-random-session-secret-32-plus-chars
WELCOME_KEY_SECRET=replace-with-random-welcome-secret-32-plus-chars
...
```text
если path не /auth/** и не /v1/** и не /health и не /ready и не /agent/** и файл не найден,
вернуть index.html
```
